...
for network, snapshots in snapshots_per_network.items():
    ...
    for artifact_type, history_mode, path in [
        ...
        typed_snapshots = [
            s
            for s in snapshots
            if s["artifact_type"] == artifact_type and s["history_mode"] == history_mode
        ]
        ...
        lowest_octez_version = sorted(
            list(set([s["tezos_version"]["version"]["major"] for s in typed_snapshots]))
        )[0]
        ...
        typed_snapshots = [
            d
            for d in typed_snapshots
            if d["tezos_version"]["version"]["major"] == lowest_octez_version
        ]
